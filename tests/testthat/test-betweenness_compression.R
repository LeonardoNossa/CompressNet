...
test_that("GraphCompression correctly compresses network using betweenness method", {
  ...
  expect_warning(
    result <- GraphCompression(Network, links, method = "betweenness", top = 0.5),
    "Compression too strong, all betweenness values are < or = threshold"
  )
...
test_that("GraphCompression handles different values for 'top' and 'size'", {
  ...
  # Test with top = 0.2
  expect_warning(
    result <- GraphCompression(Network, links, method = "betweenness", top = 0.2),
    "Compression too strong, all betweenness values are < or = threshold")
  ...
  expect_warning(
    result <- GraphCompression(Network, links, method = "betweenness", size = 50),
    "Compression too strong, all betweenness values are < or = threshold")
...
test_that("GraphCompression handles non-igraph Complete_network", {
  ...
  expect_error(GraphCompression(non_graph_object, links, method = "betweenness"),
