...
#'
#' This function takes an igraph object and compresses the network based on the betweeness centrality
#'
...
#' @importFrom igraph V E layout_with_graphopt betweenness induced_subgraph
#' @importFrom kableExtra kbl kable_styling column_spec scroll_box
#' @importFrom stats quantile
#' @importFrom graphics par
#' @return The network in compressed form based on betweenness centrality
#' @examples
#'
#' # Example usage:
...
#' # Use the compression function recalling the betweenness method
...
#' print(compression)
betweenness_centrality_compression <- function(Complete_network, links, top = 0.1, size = 25){
  ...
  if (inherits(Complete_network, "igraph") == TRUE){
    if (all(unique(as.data.frame(links)[,1]) %in% V(Complete_network)$name) && all(unique(as.data.frame(links)[,2]) %in% V(Complete_network)$name)){
      ...
      betweenness_c <- betweenness(Complete_network)     # calculus of betweenness centrality of nodes
      threshold <- quantile(betweenness_c, top)          # establish a threshold to filter the nodes
      ...
      table_betweenness <- kbl(betweenness_c, align = "c") %>%
        kable_styling() %>%
        column_spec(2, color = "black", background = ifelse(betweenness_c > threshold , "#90ee90","#ffbfaa"))

      names(threshold) <- "Threshold"
      filters_data <- data.frame(Threshold = threshold, Top = 1 - top)
      rownames(filters_data) <- "Filters"

      if(all(betweenness_c > threshold )) {
        warning("No compression, beacuse all nodes have betweenness > threshold")
        return(list(filters_data = filters_data, values = table_betweenness))
      } else if(all(betweenness_c < threshold) | all(betweenness_c == threshold)){
        warning("Compression too strong, all betweenness values are < or = threshold")
        return(list(filters_data = filters_data, values = table_betweenness))
      } else{
        ...
        compressed_nodes <- names(which(betweenness_c > threshold))
      ...
      return(list(filters_data = filters_data, values = table_betweenness, sub_graph = compressed_graph, edges = edges, edges_data = edges_data))
