...
#'
#' This function takes an igraph object and compresses the network based on the closeness centrality
#'
...
#' @importFrom igraph V E layout_with_graphopt closeness induced_subgraph
#' @importFrom kableExtra kbl kable_styling column_spec scroll_box
#' @importFrom stats quantile
#' @importFrom graphics par
#' @return The network in compressed form based on closeness centrality
#' @examples
#'
#' # Example usage:
...
#' # Use the compression function recalling the closeness method
#' compression = GraphCompression(complete_net$Complete_network,complete_net$edges_data, method = "closeness")
#' # to watch all the results obtained
#' print(compression)
closeness_centrality_compression <- function(Complete_network, links, top = 0.1, size = 25){
  ...
  if (inherits(Complete_network, "igraph") == TRUE){
    if (all(unique(as.data.frame(links)[,1]) %in% V(Complete_network)$name) && all(unique(as.data.frame(links)[,2]) %in% V(Complete_network)$name)){
      ...
      closeness_c <- closeness(Complete_network)       # calculus of the closeness centrality of nodes
      threshold <- quantile(closeness_c, top)          # establish a threshold to filter the nodes
      ...
      table_closeness <- kbl(closeness_c, align = "c") %>%
        kable_styling() %>%
        column_spec(2, color = "black", background = ifelse(closeness_c > threshold , "#90ee90","#ffbfaa"))

      names(threshold) <- "Threshold"
      filters_data <- data.frame(Threshold = threshold, Top = 1 - top)
      rownames(filters_data) <- "Filters"

      if(all(closeness_c > threshold)) {
        warning("No compression, beacuse all nodes have closeness > threshold")
        return(list(filters_data = filters_data, values = table_closeness))
      } else if(all(closeness_c < threshold) | all(closeness_c == threshold)){
        warning("Compression too strong, all closeness values are < or = threshold")
        return(list(filters_data = filters_data, values = table_closeness))
      } else{
        ...
        compressed_nodes <- names(which(closeness_c > threshold))
      ...
      return(list(filters_data = filters_data, values = table_closeness, sub_graph = compressed_graph, edges = edges, edges_data = edges_data))
